gnupgInitializeUser() {
    ...
    """ | sed -e 's/^[ \t]*//' | /usr/bin/sudo tee ${installationWorkspace}/gnupg-${userToInitialize}/initializeGpg.sh
    ...
    /usr/bin/sudo -H -i -u ${userToInitialize} bash -c 'echo "123" | gopass insert '${baseDomain}'/test' || local rc=$?
    if [[ ${rc} -ne 0 ]]; then
        # Setup GoPass
        ...
        do
            ...
            /usr/bin/sudo -H -i -u ${userToInitialize} bash -c "gopass setup --storage fs --alias kxascode --create --name \"${userToInitialize}\" --email \"${userToInitialize}@${baseDomain}\"" || local rc=$?
            if [[ ${rc} -ne 0 ]]; then
    ...
    # Insert first secret with GoPass
